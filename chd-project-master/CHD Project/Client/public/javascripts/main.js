...
function addProduct(event) {
  console.log("adding PRODUCT to the chain");
  event.preventDefault();

  const pri_key = document.getElementById("priv_key").value;
  const prod_Id = document.getElementById("prod_id").value;
  const prodName = document.getElementById("prod_name").value;
  const manu_name = document.getElementById("manu_name").value;
  const dateof_manufacture = document.getElementById("dom").value;
  if (
    pri_key != "" &&
    prod_Id != "" &&
    prodName != "" &&
    ...
    $.post(
      "/manufacturerProd",
      {
        manu_pri_key: pri_key,
        prod_id: prod_Id,
        prod_name: prodName,
        name_manu: manu_name,
        DOM: dateof_manufacture
      },
      (data, textStatus, jqXHR) => {
        alert(data.message);
      },
...
function verifyProduct(event) {
  console.log("adding new PRODUCT to the chain");
  event.preventDefault();
  let rowID = "tr" + event.srcElement.id
  let keyID = "verify_key" + event.srcElement.id
  let Row = document.getElementById(rowID)
  ...
  const pub_key = document.getElementById(keyID).value
  console.log("PK____", pub_key)
  const pri_key_dist = document.getElementById("priv_key_dist").value;
  const id_veh = document.getElementById("id_vehicle").value
  const dist_name = document.getElementById("name_distributer").value
  const prod_Id_dist = Cells[0].innerText
  const manu_name = Cells[2].innerText
  const verdict = "Verified"
  console.log(priv_key_dist, prod_Id_dist, manu_name, verdict)
  if (
    pub_key != "" &&
    pri_key_dist != "" &&
    prod_Id_dist != "" &&
    ...
    $.post(
      "/distributerItem",
      {
        Dist_pri_key: pri_key_dist,
        prod_id: prod_Id_dist,
        name_manu: manu_name,
        veh_Id: id_veh,
        name_dist: dist_name,
        Dist_verdict: verdict,
        Manu_pub_key: pub_key
      },
      (data, textStatus, jqXHR) => {
        alert(data.message);
      },
...
function getProdData(event, action = "general") {
  event.preventDefault();
  $.post(
    "/prodData",
    (data, textStatus, jqXHR) => {
      ...
      let actionButton = ""
      for (let i = 0; i < data.length; i++) {
        let ProdData = JSON.parse(data[i])
        if (action == "general") {
          tablerow = '<tr id = tr' + i + '>' + '<td>' + ProdData["Product ID"] + '</td>' + '<td>' + ProdData["Name Of Vehicle"] + '</td>' + '<td>' + ProdData["Manufacturer"] + '</td>' + '<td>' + ProdData["Date Of Manufacture"] + '<td>' + ProdData["Suppliers"] + '</td>' + '<td>' + ProdData["Shipped To"] + '</td>' + '<td>' + ProdData["Approval"] + '</td>' + '</tr>'
        } else if (action == "distributer") {
          actionButton = "<td><form><input type='text' id = verify_key" + i + " placeholder='Manufacturer Public Key'><button id = " + i + " type=\"submit\" name = \"verifyBtn\" style=\" align:center\" class=\"btn login\" onClick=verifyProduct(event)> Verify Product</button></form></td>"
          tablerow = '<tr id = tr' + i + '>' + '<td>' + ProdData["Product ID"] + '</td>' + '<td>' + ProdData["Name Of Vehicle"] + '</td>' + '<td>' + ProdData["Manufacturer"] + '</td>' + '<td>' + ProdData["Date Of Manufacture"] + '<td>' + ProdData["Suppliers"] + '</td>' + '<td>' + ProdData["Shipped To"] + '</td>' + '<td>' + ProdData["Approval"] + '</td>' + actionButton + '</tr>'
        }
        $('#assetDetails tr:last').after(tablerow);
      }
    },
...
//   const veh_id = document.getElementById("vehicle_id").value;
//   if (veh_id != "") {
//     $.post(
//       "/vehicleData",
//       {
...
//         let tablerow = ""
//        for(let i =0;i< data.length;i++){
//           let vehData = JSON.parse(data[i])
//           tablerow = '<tr id = tr'+ i + '>'+ '<td>'+vehData["Vehicle ID"] + '</td>' +  '<td>'+vehData["Distributer Name"] + '</td>' + '<td>'+vehData["Manufacturer Details"] + '<td>'+vehData["Supplier Details"]  + '</td>' +'</tr>'
//           $('#assetDetails tr:last').after(tablerow);
//        }
//       },
//       "json"
...
function delete_role(event) {
  event.preventDefault();
  let privKey = document.getElementById("priv_key_roleAdmin").value;
  let id = document.getElementById("id_type").value;
  $.post(
    "/delete",
    { key: privKey, id: id },
    function (data) {
