...
class vehicleTraceClient {
  ...
  send_ProdData(dataPayload, prodID, manufacturerName, manufacturerPublicKey) {
    try {
      const roleAddress = generateRoleAddress(this.role, this.publicKey);
      const prodDataAddress = generateProductAddress(prodID, manufacturerName, manufacturerPublicKey);
      const add = VEHICLETP["NAMESPACE"];
      const roleadd = ROLETP["NAMESPACE"];
      var inputAddressList = [add, roleadd, roleAddress, prodDataAddress];
      var outputAddressList = [add, roleadd, prodDataAddress, roleAddress];
      var encode = new TextEncoder('utf8');
      const payloadBytes = encode.encode(dataPayload);
      const transactionHeaderBytes = protobuf.TransactionHeader.encode({
        familyName: VEHICLETP["FAMILYNAME"],
  ...
  send_DistData(dataPayload, vehId, distributerName, distributerPublicKey) {
    try {
      const roleAddress = generateRoleAddress(this.role, this.publicKey);
      const vehDataAddress = generateVehicleAddress(vehId, distributerName, distributerPublicKey);
      const add = VEHICLETP["NAMESPACE"];
      const roleadd = ROLETP["NAMESPACE"];
      var inputAddressList = [add, roleadd, roleAddress, vehDataAddress];
      var outputAddressList = [add, roleadd, vehDataAddress, roleAddress];
      var encode = new TextEncoder('utf8');
      const payloadBytes = encode.encode(dataPayload);
      const transactionHeaderBytes = protobuf.TransactionHeader.encode({
        familyName: VEHICLETP["FAMILYNAME"],
  ...
  async getProd() {
    let stateRequest = 'http://rest-api:8008/state?address=' + getHash("Vehicle Trace", 6) + getHash(ROLES["MANUFACTURER"], 4);
    let stateResponse = await fetch(stateRequest);
    let stateJSON = await stateResponse.json();
    return stateJSON;
  }
  async getData(vehID) {
    let stateRequest = 'http://rest-api:8008/state?address=' + getHash("Vehicle Trace", 6) + getHash(ROLES["DISTRIBUTER"], 4) + getHash(vehID, 6);
    let stateResponse = await fetch(stateRequest);
    let stateJSON = await stateResponse.json();
    return stateJSON;
  }
